...
public class AutomaticMessages {
	...
	public static void AutoMessages() {
		...
		if (messages == 3) {
			Bukkit.getServer().broadcastMessage(prefix + "§bAplique-se para um CARGO DE STAFFER utilizando o comando /aplicar");
		}
		if (messages == 4) {
			Bukkit.getServer().broadcastMessage(prefix + "§aCansado de usar o mesmo kit? Compre um novo com suas MOEDAS na LOJA INTERNA do servidor!");
		}
		if (messages == 5) {
			Bukkit.getServer().broadcastMessage(prefix + "§2Dificuldades para conseguir MOEDAS? Tente o Lava Challenge!");
		}
		if (messages == 6) {
			Bukkit.getServer().broadcastMessage(prefix + "§6Adiquira ja o seu VIP para ter acesso a beneficios no servidor! Utilize /loja");
		}
		if (messages == 7) {
			Bukkit.getServer().broadcastMessage(prefix + "§eAcesse o site OFICIAL do servidor: www.battlebits.com.br");
		}
		if (messages == 8) {
			Bukkit.getServer().broadcastMessage(prefix + "§9Utilize o comando /rank para checar os detalhes de sua LIGA");
		}
		if (messages == 9) {
