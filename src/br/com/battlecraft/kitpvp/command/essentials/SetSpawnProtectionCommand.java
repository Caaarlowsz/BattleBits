...
public final class SetSpawnProtectionCommand extends Command {
	...
	public boolean execute(final CommandSender sender, final String label, final String[] args) {
		...
		if (args.length == 0) {
			sender.sendMessage(" ");
			sender.sendMessage("§8§lPROTEÇAO§f Utilize: /setspawnprotection <spawn/fps> (distancia)");
			sender.sendMessage(
					"§e§lEXEMPLO§f /setspawnprotection spawn 15 §e§l(setei a proteção da warp spawn para 15 blocos de distancia)");
			sender.sendMessage(" ");
			return true;
		}
		if (args.length == 1) {
			if (!args[0].equalsIgnoreCase("spawn") && !args[0].equalsIgnoreCase("fps")) {
				...
				sender.sendMessage("§8§lPROTEÇAO§f Utilize: /setspawnprotection " + args[0] + " (distancia)");
				return true;
			}
		}
		if (args.length == 2) {
