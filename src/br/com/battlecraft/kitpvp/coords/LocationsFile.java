package br.com.battlecraft.kitpvp.coords;
...
public final class LocationsFile {
	...
	public static void createLocationsFile(Plugin plugin) {
		...
		if (player_account.exists()) {
			try {
				...
				Bukkit.getConsoleSender().sendMessage("§4§lERRO§f Ao §c§lcriar§f o arquivo " + fileName);
				e.printStackTrace();
			}
		}
	...
	public static void saveWarpsFile() {
		try {
			...
			Bukkit.getConsoleSender().sendMessage("§4§lERRO§f Ao §c§lsalvar§f o arquivo " + fileName);
			e.printStackTrace();
		}
	}
}