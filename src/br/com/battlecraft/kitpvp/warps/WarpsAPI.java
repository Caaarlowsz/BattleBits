package br.com.battlecraft.kitpvp.warps;
...
import br.com.battlecraft.kitpvp.warps.Evento.EventoWarpListener;
...
public final class WarpsAPI {
	...
	public WarpsAPI(final Player bp, final String warpName) {
		if (!LocationsConstructor.checkBattleWarpNotNull(warpName)) {
			bp.sendMessage("§9§lTELEPORTE§f Este local nao foi §c§lsetado§f ou nao §c§lexiste§f!");
			return;
		}
		if (warpName.equalsIgnoreCase("Evento") && !EventoAPI.onEvent) {
			bp.sendMessage("§3§lEVENTO§f Nenhum evento esta ocorrendo.");
			return;
		}
		if (warpName.equalsIgnoreCase("Evento") && EventoAPI.Players.size() >= EventoAPI.maxEventSlots
			...
			bp.sendMessage("       ");
			bp.sendMessage("           §3§lEVENTO§f O evento esta §c§lLOTADO§f!           ");
			bp.sendMessage("           Compre seu §6§lVIP§f para ter slots da warp §3§lEvento§f garantido!   ");
			bp.sendMessage("       ");
			return;
		}
		if (warpName.equalsIgnoreCase("Evento") && EventoAPI.blocked) {
			bp.sendMessage("§3§lEVENTO§f A entrada para o §3§lEvento§f esta §c§lbloqueada§f!");
			return;
		}
		if (warpName.equalsIgnoreCase("Evento") && EventoAPI.mode == EventoAPI.EventMode.ANDAMENTO) {
			bp.sendMessage("§3§lEVENTO§f Evento ja iniciou!");
			return;
		}
		...
		if (CombatSystem.combat.containsKey(bp.getName())) {
			...
			teleporting.add(bp);
			bp.sendMessage("§9§lTELEPORTE§f Você será teleportado em §3§l5 SEGUNDOS§f. Não se mexa!");
				...
				public void run() {
					if (bp.isOnline()) {
						if (teleporting.contains(bp)) {
							...
							bp.sendMessage("§9§lTELEPORTE§f Você foi teleportado para §3§l" + warpName);
							loadWarpItensFromArgs(bp, warpName);
							TittleAPI.sendTittle(bp, "§eWarp " + warpName);
