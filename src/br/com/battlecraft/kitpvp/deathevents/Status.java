...
public final class Status {
	...
	public static final void addXpByXpToBattlePlayer(final Player bp, final Player killed) {
		...
		final int total = getXpByXp(bp, killed, kills, deaths,
				streak, xp);
	...
	public static final void AnunciarKs(final Player p) {
		...
		for (int anuncio : values) {
			if (streak == anuncio) {
				Bukkit.getServer().broadcastMessage("§4§lKILLSTREAK §1§l" + p.getName() + "§f conseguiu um §6§lKILLSTREAK DE " + streak);
				return;
			}
		}
	}
	...
	public static final void AnunciarPerdaKs(final Player p1, final Player p2) {
		...
		if (streak >= 5) {
			Bukkit.getServer().broadcastMessage("§4§lKILLSTREAK §1§l" + p1.getName() + "§fperdeu seu §6§lKILLSTREAK DE " + streak + " PARA §c§l" + p2.getName());
			return;
		}
	}
}
